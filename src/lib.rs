...
impl Tokenizer {
    ...
    pub fn decode(&self, py: Python<'_>, ids: Vec<u32>) -> PyResult<String> {
        ...
        String::from_utf8(bytes).map_err(|e| {
            // PyUnicodeDecodeError has a real 5-argument constructor
            // (encoding, object, start, end, reason) in CPython — calling
            // `new_err`/`PyErr::new` with a single formatted string, as the
            // upstream karpathy/rustbpe reference implementation currently
            // does, raises a confusing `TypeError: function takes exactly 5
            // arguments (1 given)` instead of the intended UnicodeDecodeError.
            // We build it correctly here via `PyUnicodeDecodeError::new`.
